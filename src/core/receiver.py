...
from bitrix import get_task_api, get_users_api, initialized as bitrix_initialized
...
async def receive_form(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with Database.make_session() as session:
        ...
        if not bitrix_initialized:
            ...
            logging.warning("Bitrix24 не инициализирован — задача не создана в Bitrix24")
            return {
                "ticket_id": ticket_id,
                "bitrix_task_id": None,
        ...
        if not group_id:
            status_create = Status(text="Создано (проект Bitrix24 не выбран)", ticket=ticket)
            session.add(status_create)
            return {
                "ticket_id": ticket_id,
                "bitrix_task_id": None,
        ...
        manager_name = ticket.additional_info.manager if ticket.additional_info else None
        ...
        if manager_name and users_api:
            try:
                ...
                logging.warning(f"Ошибка поиска менеджера '{manager_name}' в Bitrix24: {e}")
...
async def _notify_telegram(session, ticket: Ticket) -> None:
    try:
        chats_short = (await session.execute(
            select(TelegramConfigExtended).where(TelegramConfigExtended.created == True)
        )).scalars().all()
        for chat in chats_short:
            await TgBot.send_message(chat.chat_id, f"Получено '{ticket.title}'")

        chats_full = (await session.execute(
            select(TelegramConfigExtended).where(TelegramConfigExtended.created_full == True)
        )).scalars().all()
