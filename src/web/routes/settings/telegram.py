...
async def telegram_settings(request: Request):
    ...
    async with Database.make_session() as session:
        chats = (await session.execute(
            select(TelegramConfig).where(TelegramConfig.destination_type == 'chat')
        )).scalars().all()
        users = (await session.execute(
            select(TelegramConfig).where(TelegramConfig.destination_type == 'user')
        )).scalars().all()
...
async def telegram_settings_submit(request: Request):
    ...
    await cfg_set("notify_created", "1" if form.get("created") else "0")
    await cfg_set("notify_created_full", "1" if form.get("created_full") else "0")
    await cfg_set("notify_status_changed", "1" if form.get("status_changed") else "0")
    await cfg_set("notify_deleted", "1" if form.get("deleted") else "0")
    await cfg_set("notify_sub_tag_setted", "1" if form.get("sub_tag_setted") else "0")
    await cfg_set("notify_commented", "1" if form.get("commented") else "0")
    ...
    async with Database.make_session() as session:
        ...
        for chat in chats:
            ...
            if chat and chat.startswith("-"):
                session.add(TelegramConfig(destination_id=chat, destination_type='chat'))
        for user in users:
            user = user.strip()
            if user and user.startswith("@"):
                session.add(TelegramConfig(destination_id=user, destination_type='user'))
...
async def new_chat_settings_page(request: Request):
    ...
    projects = await get_watched_projects()
    return settings_template("telegram/chat_settings.html", {
        "request": request,
...
async def submit_new_chat_settings(request: Request):
    ...
    if not ok:
        ...
        async with Database.make_session() as session:
            existing = (await session.execute(
                select(TelegramConfigExtended).where(TelegramConfigExtended.chat_id == chat_id)
            )).scalars().all()
            if any(r.additional == project_id for r in existing):
                ...
                return RedirectResponse(telegram_settings_router.prefix + "/new-chat-settings",
            ...
            setting = TelegramConfigExtended(
                chat_id=chat_id, created=created, created_full=created_full,
                status_changed=status_changed, deleted=deleted,
                sub_tag_setted=sub_tag_setted, commented=commented,
    ...
    if setting is None:
        return RedirectResponse(telegram_settings_router.prefix + "/new-chat-settings",
                                status_code=HTTP_303_SEE_OTHER)
    return RedirectResponse(telegram_settings_router.prefix + f"/chat-settings/{setting_id}",
                            status_code=HTTP_303_SEE_OTHER)
...
async def show_chat_settings_page(request: Request, setting_id: int):
    success_message = request.session.pop("new_tg_chat_success_message", None)
    async with Database.make_session() as session:
        chat_setting = (await session.execute(
            select(TelegramConfigExtended).where(TelegramConfigExtended.id == setting_id)
        )).scalars().one_or_none()
    projects = await get_watched_projects()
    return settings_template("telegram/chat_settings.html", {
        "request": request,
